...
```text
OmniGlyph 是一个本地优先的 MCP server 和 Python 包，帮助 AI Agent 在推理、输出或请求工具动作前，对 Unicode、Unihan、私有领域术语、安全字符风险、输出术语和意图策略做确定性检查。
```
...
- SQLite 本地事实库
- PyPI 包：`omniglyph`
- 数据输入：UnicodeData、Unihan、CSV domain packs、Lexicon Packs、Policy Packs
...
- MCP 工具：`lookup_glyph`、`lookup_term`、`explain_glyph`、`explain_term`、`explain_code_security`、`normalize_tokens`、`validate_lexicon_pack`、`validate_policy_pack`、`validate_output_terms`、`enforce_grounded_output`、`scan_unicode_security`、`scan_language_input`、`scan_output_dlp`、`enforce_intent`、`audit_explain`；`scan_code_symbols` 保留为向后兼容别名。
...
```bash
.venv/bin/omniglyph init-policy-pack my-policy --namespace private_acme --policy-id company.acme.agent_policy --name "ACME Agent Policy"
.venv/bin/omniglyph validate-policy-pack my-policy
.venv/bin/omniglyph enforce-intent example.review --policy-pack my-policy --actor-role admin --parameters '{"ticket":"123"}'
```

示例 Policy Pack 位于 `examples/policy-packs/agent_intents`。API 和 MCP 可以通过 `policy_pack_path` 使用它；旧的 inline manifest 方式仍保持兼容。
