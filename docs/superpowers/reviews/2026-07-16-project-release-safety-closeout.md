...
最终结果：新增 16 个回归测试，共 205 项测试通过；完整发布门禁通过；公开 API、
...
| 中 | `glyph_property` 唯一约束包含可空 `language` | SQLite 对 `NULL` 的唯一性语义允许完全相同的属性重复导入 | 属性 ID 改为基于结构化完整来源身份的 UUID5，重复导入保持幂等且字段边界不可混淆 |
